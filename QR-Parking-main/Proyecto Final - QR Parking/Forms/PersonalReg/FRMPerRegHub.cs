...
namespace Proyecto_Final___QR_Parking.PersonalReg
{
	public partial class FRMPerRegHub : Form
	{
		...
		private void dgvRegistros_CellContentClick(object sender, DataGridViewCellEventArgs e)
		{
            try
            {
                // Desactiva el dgv para evitar problemas de concurrencia
                dgvRegistros.Enabled = false;

                // Validar que el clic ocurrió en una fila válida
                if (e.RowIndex < 0 || e.RowIndex >= dgvRegistros.Rows.Count)
                    return;

                // Validar que el clic ocurrió en una columna válida
                if (e.ColumnIndex < 0 || e.ColumnIndex >= dgvRegistros.Columns.Count)
                    return;

                // Obtener la fila seleccionada
                DataGridViewRow filaSeleccionada = dgvRegistros.Rows[e.RowIndex];

                // Obtener los datos de la fila seleccionada con validaciones
                ...
                // Manejo de acciones específicas según la columna
                ...
                {
                    var placaQRCell = filaSeleccionada.Cells["PlacaQR"];
